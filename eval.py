...
# ===================== 配置 =====================
dataset     = "6"
depth_dir   = f"./data/halfDef/{dataset}/depth"              # 深度图文件夹（含 0.png 模板与 1.png...）
mesh_dir    = f"./data/halfDef/{dataset}/results/PLYs"       # 变形后网格文件夹（deformed_mesh_f1.ply ...）
poses_txt   = f"./data/halfDef/{dataset}/poses_gt.txt"       # 每行 12(3x4) 或 16(4x4)，与深度图索引一致
pose_format = "tcw"                                          # "tcw" 或 "twc"
mesh_unit   = "mm"                                           # PLY 单位："mm" 或 "m"

# ✅ 新增：模板（初始）网格路径（obj/ply 都可）
# 例如 "./data/halfDef/6/template.obj" 或 "./data/halfDef/6/template.ply"
...
def load_poses_multi(path, pose_format="tcw"):
    # 返回按行索引的 T_wc（行 0 对应模板，1..N 对应各帧）
    try:
...
if __name__ == "__main__":
    ...
    # 读取多帧位姿（返回 T_wc 列表，行号与深度图索引一致）
    T_wcs = load_poses_multi(poses_txt, pose_format=pose_format)
    ...
    # 建立索引->路径映射，遍历匹配 index>=1（跳过模板 0）
    depth_map = dict(depth_files)
    mesh_map  = dict(mesh_files)
    max_idx = min(max(depth_map.keys()), max(mesh_map.keys()), len(T_wcs)-1)

    print(f"[INFO] depth imgs: {len(depth_files)}, meshes: {len(mesh_files)}, poses: {len(T_wcs)} (max usable idx={max_idx})")
    print("[INFO] 从 index=1 开始计算（index=0 视为模板并跳过）")
    ...
    for idx in range(1, max_idx + 1):
        ...
        if V_def_m is None:
            ...
            depth_def_m = render_depth_from_mesh(V_def_m, F_def, T_wc, H, W)
            rmse_def_m, npx_def, sse_def = rmse(depth_def_m, depth_m)

        # ----- Template vs Depth -----
        depth_tmpl_m = render_depth_from_mesh(V_tmpl_m, F_tmpl, T_wc, H, W)
