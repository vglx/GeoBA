...
std::pair<float, float> ImageProcessor::computeGradient(const cv::Mat& image, double u, double v) {
    ...
    float Ix1 = getBilinearInterpolatedValue(image, u + eps, v);
    float Ix2 = getBilinearInterpolatedValue(image, u - eps, v);
    float Iy1 = getBilinearInterpolatedValue(image, u, v + eps);
    float Iy2 = getBilinearInterpolatedValue(image, u, v - eps);
