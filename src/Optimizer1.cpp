...
void Optimizer::optimize(const std::vector<MeshModel::Vertex>& mesh_vertices,
    ...
    for (size_t i = 0; i < frame_count; ++i) {
        ...
        ceres::LossFunction* robust_kernel = new ceres::HuberLoss(1.0);
        problem.AddResidualBlock(cost_function, robust_kernel, &poses[i * 6]);
    }
    ...
    for (size_t i = 1; i < frame_count - 1; ++i) {
        ...
        ceres::LossFunction* robust_kernel = new ceres::HuberLoss(1.0);
        problem.AddResidualBlock(cost_function, robust_kernel, &poses[i * 6], &poses[(i - 1) * 6], &poses[(i + 1) * 6]);
    }
