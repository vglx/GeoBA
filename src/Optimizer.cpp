...
#include "BVH.h"  // kept for constructor signature compatibility; Evaluate currently ignores BVH
...
// =====================================================================================
//  Behavior:
//  - Frame 0 is FIXED (no ED columns), but contributes DATA residual rows.
//  - Per-vertex photometric map intensity I_i is a variable (1 scalar / vertex).
//  - I_i initialization uses mean from frame0 & frame1 (when visible),
//    falling back to single-frame if only one is visible.
//  - Optional soft prior E_prior = lambda_I * (I_i - I_prior)^2. (lambda_I set below)
//  - ED variables are built for frames f>=1 (active nodes only).
//  - NEW: Each GN iteration RE-COMPUTES visibility with boundary-only check (no occlusion).
// =====================================================================================
...
void Optimizer::optimize(
    ...
    // =========================
    // Per-frame ED full state (12*G) for evaluation; frame 0 remains fixed
    // =========================
    ...
    // =====================================================================================
    // (A) ONE-TIME: Build a dummy BVH per frame for constructor compatibility (Evaluate ignores it)
    // =====================================================================================
    std::vector<std::vector<MeshModel::Vertex>> Vdef_init(F, std::vector<MeshModel::Vertex>(N));
    std::vector<BVH> bvhs(F, BVH(mesh_triangles, Vdef_init[0]));
    for (int f = 0; f < (int)F; ++f) {
        edGraph.updateFromStateVector(Xfull[f], /*offset=*/0);
        auto& Vdef = Vdef_init[f];
        ...
        bvhs[f] = BVH(mesh_triangles, Vdef); // harmless even if unused
    }

    // =====================================================================================
    // (B) ONE-TIME: Visibility for f=0 and f=1 to initialize I_i (map intensity)
    // =====================================================================================
    ...
    // =========================
    // (C) Map intensity initialization (I_init) and prior (I_prior) from frame0 & frame1
    // =========================
    ...
    {
        const double w0 = 0.5, w1 = 0.5; // mean; swap to median if needed
        ...
        for (int i = 0; i < (int)N; ++i) {
            ...
            if (m0[i]) {
                ...
                if (pc.z() > 1e-8) {
                    float u = (float)(K(0,0) * (pc.x()/pc.z()) + K(0,2));
                    float v = (float)(K(1,1) * (pc.y()/pc.z()) + K(1,2));
                    v0 = (double)bilinearSample(imgs_gray[0], u, v);
                }
            }
            if (m1[i]) {
                Eigen::Vector3d pw = edGraph.deformVertex(mesh_vertices[i], i);
                ...
                if (pc.z() > 1e-8) {
                    float u = (float)(K(0,0) * (pc.x()/pc.z()) + K(0,2));
                    float v = (float)(K(1,1) * (pc.y()/pc.z()) + K(1,2));
                    v1 = (double)bilinearSample(imgs_gray[1], u, v);
                }
            }
            bool ok0 = std::isfinite(v0), ok1 = std::isfinite(v1);
            if (ok0 && ok1) { I_init[i] = I_prior[i] = (w0*v0 + w1*v1); }
    ...
    // =========================
    // (D) Build intensity variable mapping (fixed across iterations)
    //      We give a column to any vertex with finite prior (or you can require visibility too)
    // =========================
    std::vector<int> colI(N, -1);
    int Icount = 0;
    for (int i = 0; i < (int)N; ++i) if (std::isfinite(I_prior[i])) colI[i] = Icount++;

    std::vector<double> I_var(Icount, 0.0);
    for (int i = 0; i < (int)N; ++i) if (colI[i] >= 0) I_var[colI[i]] = std::isfinite(I_init[i]) ? I_init[i] : 0.0;

    // =========================
    // (E) Gauss‑Newton with per-iteration visibility refresh
    // =========================
    const double lambda_I = 0.0; // set >0 for a soft prior on intensities

    double prev_cost = std::numeric_limits<double>::max();

    for (int it = 0; it < maxIterations_; ++it) {
        // --- E1) Recompute visibility for ALL frames under current Xfull (boundary + depth only)
        ...
        // --- E2) Build active nodes (f>=1) and compact mapping according to current visibility
        ...
        for (int f = 1; f < (int)F; ++f) {
            for (int vid : visible_vertices[f]) {
                if (colI[vid] < 0) continue; // only vertices with an intensity variable
        ...
        // --- E3) Row layout with CURRENT visibility
        ...
        // --- E4) Assemble normal equations under CURRENT visibility/mapping
        ...
        for (int f = 0; f < (int)F; ++f) {
            ...
            const BVH& bvh = bvhs[f]; // not used by Evaluate currently
            ...
            for (int idx = 0; idx < (int)visible_vertices[f].size(); ++idx) {
                ...
                double residual = 0.0; 
                double J_I = 0.0; // d r / d I_i
                Eigen::VectorXd J_ed(12 * G); J_ed.setZero();

                cost.Evaluate(Icurr, residual,
                              /*jacobian_intensity=*/&J_I,
        ...
        if (cost > prev_cost * (1.0 - 1e-9)) {
            // Consider adding LM damping or backtracking for extra robustness
            break;
        }
        prev_cost = cost;
        ...
        // loop continues: next iteration will recompute visibility under updated Xfull
    }

    // (Optionally) export results; map intensities I_var can be written back if needed.
}