...
void Optimizer::optimize(
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        ...
        for (size_t j = 0; j < frame_count; ++j) {
            if (Projection::isVertexVisible(mesh_vertices[i], camera_intrinsics,
                camera_poses[j].block<3,3>(0,0), camera_poses[j].block<3,1>(0,3),
                bvh, observed_images[j].cols, observed_images[j].rows)) {

                // **创建残差项**
                ceres::CostFunction* photometric_cf = MultiViewPhotometricError::Create(
                    mesh_vertices[i], mesh_triangles, camera_intrinsics, observed_images[j], bvh, weight_
                );

                // **添加到 Ceres 优化问题**
                problem.AddResidualBlock(photometric_cf, nullptr, &poses[j * 6]);
